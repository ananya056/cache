...
/**
 * @brief A transpose function optimized for a 32 x 32 matrix.
 *
 * This is optimized by going through the matrix in 8 x 8 blocks and delaying
 * the diagonals to reduce double evictions from reading and writing to the
 * same set.
 */
...
/**
 * @brief A transpose function optimized for a 1024 x 1024 matrix.
 *
 * This is optimized by going through the matrix in 8 x 8 blocks and delaying
 * the diagonals to reduce double evictions from reading and writing to the
 * same set. Further, there is an offset when writing to tmp so that we are not
 * accessing the same set when writing to tmp and reading from A.
 */
static void trans_1024(size_t M, size_t N, double A[N][M], double B[M][N],
                       double tmp[TMPCOUNT]) {
    /* The diag variable signifies if the current block
    is on the diagonal and thereby has numbers on the diagonal
    of the matrix so that we can delay those operations.
    */
    int diag;
    for (size_t i = 0; i < N; i += BLOCKSIZE) {
        for (size_t j = 0; j < M; j += BLOCKSIZE) {
            for (size_t ii = i; ii < i + BLOCKSIZE; ++ii) {
                ...
                for (size_t jj = j; jj < j + BLOCKSIZE; ++jj) {
                    if (ii != jj) {
                        ...
                        diag = 1;
                        tmp[(ii + BLOCKSIZE) % TMPCOUNT] = A[ii][ii];
                    }
                }
                if (diag == 1) {
                    B[ii][ii] = tmp[(ii + BLOCKSIZE) % TMPCOUNT];
                }
            }
        }
    }
